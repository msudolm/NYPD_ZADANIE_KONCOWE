...
def filter_start_end(args, dfs,  prt_sizes=print_df_sizes):
	if args.start is not None or args.end is not None:
		...
		if args.start is not None: dfs = [df.loc[df.index.astype(int) >= int(args.start)] for df in dfs]
		if args.end is not None: dfs = [df.loc[df.index.astype(int) <= int(args.end)] for df in dfs]			
...
def run_analysis():
	...
	args = parse_args()	
	if args is None:
		print("Brak poprawnie przekazanych argumentów. Kończę wykonywanie programu.")
		return
